...
export const ContractInteraction = (props) => {
  const battleAddress = props.collectionBattle.address
  ...
  const tokenIds = props.collectionBattle.tokenIds
  ...
  const handleCollectionBattleDateChange = (newDate) => {
    setCollectionBattleDate(new Date(newDate).toISOString())
  }
          ...
          ([
            ...
            if (polygonContract && polygonContract.provider && polygonAbi.length !== 0) {
              ...
              if (
                ...
                queueId &&
                new Date(props.collectionBattle.battleDate) < new Date(Date.now() + 600000)
              ) {
  ...
  const updateCollectionBattleTime = async () => {
    if (chainId === parseInt(process.env.NEXT_PUBLIC_DEFAULT_ETHEREUM_NETWORK_CHAIN_ID)) {
      if (account === owner) {
        toastInProgress()
        const tx = await ethereumInjectedContract.setStartingTime(
          Date.parse(new Date(collectionBattleDate)) / 1000
        )
        await tx.wait()
        toastCompleted()
      } else {
    ...
    <>
      ...
      <Grid container spacing={1}>
        <Grid item md={6}>
          ...
          <Card>
            <CardHeader title="Set CollectionBattle Time" sx={{ py: 1 }} />
            <Divider />
            <CardContent>
              <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateTimePicker
                  label="CollectionBattle Date"
                  value={collectionBattleDate}
                  onChange={handleCollectionBattleDateChange}
            ...
            >
              <Button
                color="primary"
                variant="contained"
                onClick={updateCollectionBattleTime}
                disabled={battleState !== 0 ? true : false}
              >
                Update
              </Button>
